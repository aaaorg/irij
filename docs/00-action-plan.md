...
- [x] V `server/data/merchant_tables.json` definovat tabulky pro kovář + general store
- [x] Server: `SHOP_OPEN`, `SHOP_BUY`, `SHOP_SELL` handlery
- [x] Server: stock respawn tick (15 min interval)
- [x] Server: buy_limit_per_day reset
- [x] Klient: shop UI — vlevo NPC stock, vpravo hráč inventář, tlačítka
- [x] **Demo:** prodej rudu kováři vs. general store, vidět rozdílné ceny
...
- **2026-05-08** — **Phase 13 dokončena**. Shared: `shared/src/types/shop.ts` (`MerchantTableDefinition` + `MerchantSellEntryDefinition` + `MerchantBuyEntryDefinition` + runtime `MerchantTableState` se stock_current/stock_partial decimal accumulator + buy_consumed_today + buy_limit_reset_date), `shared/src/messages/shop.ts` (`ShopOpenRequest`/`ShopBuyRequest`/`ShopSellRequest` + `ShopOpen` snapshot s `ShopSellEntryView`/`ShopBuyEntryView` + `ShopRejected` s 12 reason kódy). Nové opcody 93-96: SHOP_OPEN, SHOP_BUY, SHOP_SELL, SHOP_REJECTED. `DialogEffect` rozšířen o `open_shop { merchant_table_id }`. Nová konstanta `SHOP_STOCK_RESPAWN_INTERVAL=9000` (15 min). Server data: `server/data/merchant_tables.json` — 2 tabulky: `merchant.kovar_blatiny` typ `specialist.smithing` (sell: bronz dýka/meč/palcát + krumpáč/sekera/kladivo + 20× whetstone; buy: iron 18d / copper 8d / flint 2d s daily limity 100/100/200), `merchant.selka_general` typ `general` (sell: chleba 8d×20 / raw_meat 3d×10; buy: iron 10d / copper 5d / flint/bone/hide.wolf/hide.rat/wood.oak za nižší ceny — fallback kupec). Selka NPC: `merchant: true` flag + `merchant_table_id`. Kovář dialog `kovar_blatiny.json` — option "Co máš na prodej?" (Phase 9 give_item whetstone) přejmenován na "Ukaž mi, co kováš." s `open_shop` effectem; Selka dialog rozšířen o "Co prodáváš a co kupuješ?" option také s open_shop. Server: `server/src/lib/merchants.ts` static catalog (per-id + per-NPC index), `server/src/match/shop.ts` engine — `seedMerchantTables` v matchInit (stock_current = stock_max), `runShopStockRespawn` (proporcionální dopočet podle `respawn_per_hour` × hodiny od `last_respawn_tick`, partial accumulator pro float frakce, cap na stock_max), `ensureBuyLimitReset` (lazy at-handler check vs YYYY-MM-DD UTC), `sendShopOpen` (volá se z dialog effect open_shop, projektuje sell+buy entries do view), `handleShopBuy` (rate limit 5/s, range ≤ 2 k NPC, stock check, OCC retry s atomic deduct denáry + add item, fail-reason inference: insufficient_funds vs inventory_full, success → stock--, INVENTORY_CHANGED unicast + re-snapshot SHOP_OPEN), `handleShopSell` (rate, range, NPC kupuje tento item check, buy_limit_per_day + consumed gate, OCC s atomic deduct item + add denáry, success → buy_consumed_today++, broadcast). MAX_TX_QUANTITY=1000 anti-grief. `state.ts` rozšířen o `merchantStates: { [tableId]: MerchantTableState }`. `world.ts` matchInit volá `seedMerchantTables`, matchLoop dispatchuje SHOP_BUY/SELL + volá `runShopStockRespawn` každých `SHOP_STOCK_RESPAWN_INTERVAL` (15 min). `dialog.ts` `applyDialogEffect` case `open_shop` (resolve npcInstanceId → sendShopOpen). Klient: `client/src/ui/ShopPanel.ts` DOM overlay (centered modal s title+meta, dvousloupcový grid: vlevo „Prodává" sell rows s +1/+5 buy buttony zakázanými při stock=0, vpravo „Kupuje" buy rows s −1/−5/−all sell buttony + counter „mám N", `getInventoryCount` callback pro live counts, lokální `mapRejectReason` mapuje 12 reasons na české toasty). `WorldScene` napojení (lazy `ensureShopPanel`, SHOP_OPEN handler zavírá dialog + zobrazí panel, SHOP_REJECTED → toast s českou hláškou, `onInventoryChanged` propagace pro live update sell counterů). 21 nových server unit testů v `shop.test.ts` (catalog 4× — loader + per-id + per-NPC + price asymmetry specialist > general; seedMerchantTables 1×; runShopStockRespawn 3× — cap, refill from 0 po hodině, partial accumulation cross-call; handleShopBuy 5× — happy path s deduct denáry + INVENTORY_CHANGED + re-snapshot, out_of_range, not_for_sale, out_of_stock, insufficient_funds; handleShopSell 5× — happy path 5×iron→90d, not_buying, buy_limit_reached, inventory_short, specialist Kovář pays 18d (vs Selka general 10d ověřeno přes catalog 4. test); sendShopOpen 2× — payload struktura, unknown_table reject). Updated dialog test (kovář shop option teď open_shop místo give_item). Updated jobboard smoke (Selka má 4 options: shop+job_board+weather+exit). Updated dialog smoke (kovář option-flow přepnut na "Jak se vede?" → smalltalk_node, protože "Co máš na prodej?" teď otevírá ShopPanel a opouští dialog). Nový Playwright `shop.spec.ts` smoke (Kovář dialog → "Ukaž mi" option → SHOP_OPEN → panel obsahuje "Bronzový meč 120 d" + "Železná ruda 18 d" + close X). Celkem **79 shared + 216 server testů + 12 Playwright smoke**. Parking lot pro post-MVP: persistence merchant stock přes server restart, percentuální discount per reputation tier, NPC trh manažer s player listings.
